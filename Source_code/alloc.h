 ...
 *
 * @details Les macros CREATE_MEMORY(), CREATE_ZERO_MEMORY() et
 * REDIMENSION_MEMORY() définies dans ce fichier permettent d'allouer de la
 * mémoire dynamiquement, en vérifiant que l'allocation a réussi. En cas
 * d'échec, elles affichent un message d'erreur sur la sortie erreur, en
 * indiquant la fonction, le fichier et la ligne où l'allocation a échoué, et
 ...
 * ```
 * backmove *p;
 * CREATE_MEMORY(p, 10); // Alloue un tableau de 10 backmove.
 * ```
 *
 ...
 * REDIMENSION_MEMORY.
 *
 * @see REDIMENSION_MEMORY()
 */

#define CREATE_MEMORY(p, num_objects)                                          \
    do {                                                                       \
 ...
 * ```
 * backmove *p;
 * CREATE_ZERO_MEMORY(p, 10); // Alloue un tableau de 10 backmove initialisé à 0
 * ```
 *
 * Si l'allocation échoue, affiche un message d'erreur et quitte le programme.
 *
 ...
 * REDIMENSION_MEMORY.
 *
 * @sa REDIMENSION_MEMORY()
 */

#define CREATE_ZERO_MEMORY(p, num_objects)                                     \
    do {                                                                       \
 ...
 * CREATE_MEMORY(p, 10); // Alloue un tableau de 10 backmove.
 *                       // ...
 * REDIMENSION_MEMORY(p, 20); // Ré-alloue un tableau de 20 backmove en copiant
 *                            // le tableau précédent en début ce nouveau
 *                            // tableau, et affecte à p l'adresse de la
 *                            // première case de ce tableau.
 * ```
 *
 * Si l'allocation échoue, affiche un message d'erreur et quitte le programme.
 *
 ...
 *
 * @sa CREATE_MEMORY(), CREATE_ZERO_MEMORY()
 */

#define REDIMENSION_MEMORY(p, num_objects)                                     \
    do {                                                                       \
 ...
 * int *p1, *p2, *p3;
 *
 * CREATE_MEMORY(p1, 10); // Alloue un tableau de 10 int.
 * CREATE_MEMORY(p2, 10);
 * CREATE_MEMORY(p3, 10);
 * // ...
